...
def build_start_text(transport: Any, *, chat_id: int) -> str:
    ...
    lines = [
        ...
        "В личке отвечаю на любой текст. В группе — на команды, обращения @smart_therm_bot и ответы на мои сообщения. "
        "",
        "Поддерживаю ручное сохранение фактов о пользователе в формате «Ключ=Значение» (например, «Контроллер=ESP32»)",
        "",
        "<b>Команды:</b>",
        "<pre>",
        *(
